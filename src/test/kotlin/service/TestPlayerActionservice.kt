 package service
    ...
    class PlayerActionServiceTest {
        ...
        private fun setUpGame(): RootService {
            val rootService = RootService(player1 , player2 , pyramid )
            val game= rootService.gameService

            game.startGame()
            val pyramide = rootService.currentGame
            checkNotNull(pyramide)
            pyramide.cards[1]?.set(0, Card(CardSuit.CLUBS, CardValue.SEVEN))
            pyramide.cards[1]?.get(0)?.isRevealed = true
            pyramide.cards[3]?.set(2, Card(CardSuit.CLUBS, CardValue.EIGHT))
            pyramide.cards[3]?.get(2)?.isRevealed = true
            pyramide.cards[4]?.set(0, Card(CardSuit.HEARTS, CardValue.ACE))
            pyramide.cards[4]?.get(0)?.isRevealed = true
            pyramide.cards[5]?.set(0, Card(CardSuit.HEARTS, CardValue.TWO))
            pyramide.cards[5]?.get(0)?.isRevealed = true
         ...
         */
        @Test
        fun testRemovePair(): RootService {

             setUpGame()
            val rootService1 = RootService(player1 , player2 , pyramid )
            val pyramide = rootService1.currentGame
            checkNotNull(pyramide)
            val game = rootService1.gameService
            ...
            rootService1.playerActionService.removePair(sevenCard, eigthCard)
            ...
            rootService1.playerActionService.removePair(twoCard, reserveAceCard)
